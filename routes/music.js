...
router.get('/song/:"title"', async (req, res, next) => {
    ...
    try {
        ...
        for (var i = 0; i < song.length; i++) {
            ...
            var book = await db.pool.query(
                "SELECT * FROM book WHERE book_id = " + song[i].book_id);

           // use artist_id to get artist name
            var artist = await db.pool.query(
                "SELECT * FROM artist WHERE artist_id = " + song[i].artist_id);

            // use band_id to get band name
            var band = await db.pool.query(
                "SELECT * FROM band WHERE band_id = " + song[i].band_id);
...
router.post("/", async (req, res, next) => {
    ...
    if (book.length == 0 || music.length == 0) {
        ...
        try {
            ...
            let songid = 0;
            result = await db.pool.query("INSERT INTO song(songtitle, composer, book_id, band_id, artist_id) values(?, ?, ?, ?, ?) RETURNING song_id", [songtitle, composername, bookid, bandid, artistid]);
